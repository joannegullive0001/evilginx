...
// setManagedSyncWildcard obtains wildcard certificates using DNS-01 challenge
// This prevents phishing hostnames from appearing in Certificate Transparency logs
func (o *CertDb) setManagedSyncWildcard(wildcardHosts []string, t time.Duration) error {
	// Check if external DNS is configured
	if !o.ns.IsExternalDNSEnabled() {
		return fmt.Errorf("wildcard certificates require external DNS to be enabled and configured")
	}
	
	extDNS := o.ns.GetExternalDNS()
	provider := extDNS.GetProvider()
	if provider == nil {
		return fmt.Errorf("DNS provider not configured")
	}
	
	// Create our DNS solver that wraps the DNS provider
	dnsSolver := NewDNSSolver(provider, o.cfg)
	
	// Use certmagic's DNS01Solver which wraps our ACMEDNSProvider
	dns01Solver := &certmagic.DNS01Solver{
		DNSProvider:        dnsSolver,
		TTL:                120 * time.Second,
		PropagationDelay:   10 * time.Second,
		PropagationTimeout: 5 * time.Minute,
	}
	
	// Configure certmagic to use DNS-01 challenge
	acmeIssuer := certmagic.NewACMEIssuer(o.magic, certmagic.ACMEIssuer{
	...
	o.magic.Issuers = []certmagic.Issuer{acmeIssuer}
	
	log.Info("[certs] obtaining wildcard certificates via DNS-01 challenge...")
	log.Info("[certs] hostnames: %v", wildcardHosts)
	
	ctx, cancel := context.WithTimeout(context.Background(), t)
	defer cancel()
	
	err := o.magic.ManageSync(ctx, wildcardHosts)
	if err != nil {
		return fmt.Errorf("failed to obtain wildcard certificates: %v", err)
	}
	
	log.Success("[certs] wildcard certificates obtained successfully")
	return nil
}

// GetCertificateHosts returns the appropriate hostnames for certificate management
// based on whether wildcard mode is enabled
func (o *CertDb) GetCertificateHosts() ([]string, bool) {
	if o.cfg.IsWildcardCertsEnabled() && o.ns.IsExternalDNSEnabled() {
		// Use wildcard hostnames
		return o.cfg.GetWildcardHostnames(), true
	}
	// Use regular hostnames
	return o.cfg.GetActiveHostnames(""), false
}
...
// GetNameserver returns the nameserver instance
func (o *CertDb) GetNameserver() *Nameserver {
	return o.ns
}
